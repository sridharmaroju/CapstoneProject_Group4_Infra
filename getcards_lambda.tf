resource "aws_lambda_function" "getCards_lambda" {
  ...
  function_name = "${var.name_prefix}-get-cards-api-${local.workspace_safe}"
  description   = "Lambda function to get Cards from DB for -${local.workspace_safe}"
  runtime       = "python3.12"
  handler       = "GetCards.lambda_handler"
  role          = aws_iam_role.lambda_exec.arn
  memory_size   = 256 # Increase memory (default is 128 MB)
  timeout       = 30  # Increase timeout in seconds (default is 3)

  filename = "${path.module}/Lambda/GetCardsCode.zip"
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.getCards_lambda_sg.id]
  }
...
resource "aws_cloudwatch_log_group" "getCards_lambda_log" {
  ...
  name              = "/aws/lambda/${aws_lambda_function.getCards_lambda.function_name}"
  retention_in_days = 7
}

resource "aws_security_group" "getCards_lambda_sg" {
  # checkov:skip=CKV_AWS_23: "Ensure every security group and rule has a description"
  # checkov:skip=CKV2_AWS_5: "Ensure that Security Groups are attached to another resource"
  name        = "${var.name_prefix}-getCards-lambda-sg-${local.workspace_safe}"
  description = "Security group for Get Cards Lambda functions"
...
resource "aws_security_group_rule" "egress_getCards_lambda_to_sm_endpoint" {
  # checkov:skip=CKV_AWS_23: "Ensure every security group and rule has a description"
  type              = "egress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
  security_group_id = aws_security_group.getCards_lambda_sg.id
