resource "aws_lambda_function" "getTransactionHistory_lambda" {
  ...
  function_name = "${var.name_prefix}-get-transHist-api-${local.workspace_safe}"
  description   = "Lambda function to get Transaction History from DB for -${local.workspace_safe}"
  runtime       = "python3.12"
  handler       = "GetTransactionHistory.lambda_handler"
  role          = aws_iam_role.lambda_exec.arn
  memory_size   = 256 # Increase memory (default is 128 MB)
  timeout       = 30  # Increase timeout in seconds (default is 3)

  filename = "${path.module}/Lambda/GetTransactionHistoryCode.zip"
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.getTransactionHistory_lambda_sg.id]
  }
...
resource "aws_cloudwatch_log_group" "getTransactionHistory_lambda_log" {
  ...
  name              = "/aws/lambda/${aws_lambda_function.getTransactionHistory_lambda.function_name}"
  retention_in_days = 7
}
...
resource "aws_security_group_rule" "egress_getTransactionHistory_lambda_to_sm_endpoint" {
  # checkov:skip=CKV_AWS_23: "Ensure every security group and rule has a description"
  type              = "egress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
  security_group_id = aws_security_group.getTransactionHistory_lambda_sg.id
