...
resource "aws_api_gateway_integration" "deduct_integration" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.deduct.id
  ...
  uri = "arn:aws:apigateway:${data.aws_region.current.name}:sqs:path/${data.aws_caller_identity.current.account_id}/${aws_sqs_queue.deduct_queue.name}"
...
resource "aws_api_gateway_method_response" "deduct_post_response_200" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.deduct.id
  http_method = aws_api_gateway_method.deduct_post.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "deduct_card_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.deduct.id
  http_method = aws_api_gateway_method.deduct_post.http_method

  # 1. The HTTP status code the client should receive (200 OK)
  status_code = aws_api_gateway_method_response.deduct_post_response_200.status_code
  ...
  response_templates = {
    "application/json" = <<-EOF
                            {
                                "status": "success",
                                "message": "Deduct request successfully queued.",
...
resource "aws_api_gateway_method" "deduct_options" {
...
resource "aws_api_gateway_integration" "deduct_options_mock" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.deduct.id
  http_method = aws_api_gateway_method.deduct_options.http_method
  type        = "MOCK"
...
resource "aws_api_gateway_method_response" "deduct_options_response_200" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.deduct.id
  http_method = aws_api_gateway_method.deduct_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "deduct_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.deduct.id
  http_method = aws_api_gateway_method.deduct_options.http_method
  status_code = aws_api_gateway_method_response.deduct_options_response_200.status_code
