resource "aws_lambda_function" "addcard_lambda" {
  ...
  function_name = "${var.name_prefix}-add-card-api-${local.workspace_safe}"
  description   = "Lambda function to add card into DB for -${local.workspace_safe}"
  runtime       = "python3.12"
...
resource "aws_cloudwatch_log_group" "addcard_lambda_log" {
  ...
  name              = "/aws/lambda/${aws_lambda_function.addcard_lambda.function_name}"
  retention_in_days = 7
}
...
resource "aws_security_group_rule" "egress_lambda_to_sm_endpoint" {
  # checkov:skip=CKV_AWS_23: "Ensure every security group and rule has a description"
  type              = "egress"
  from_port         = 443
  to_port           = 443
