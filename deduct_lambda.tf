resource "aws_lambda_function" "deduct_lambda" {
  ...
  function_name = "${var.name_prefix}-deduct-api-${local.workspace_safe}"
  description   = "Lambda function to deduct card in DB for -${local.workspace_safe}"
  runtime       = "python3.12"
  handler       = "Deduct.lambda_handler"
  role          = aws_iam_role.lambda_exec.arn
  memory_size   = 256 # Increase memory (default is 128 MB)
  timeout       = 30  # Increase timeout in seconds (default is 3)

  filename = "${path.module}/Lambda/DeductCode.zip"
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.deduct_lambda_sg.id]
  }
  ...
  depends_on = [aws_lambda_function.topup_lambda]
}

resource "aws_cloudwatch_log_group" "deduct_lambda_log" {
  ...
  name              = "/aws/lambda/${aws_lambda_function.deduct_lambda.function_name}"
  retention_in_days = 7
}

resource "aws_security_group" "deduct_lambda_sg" {
  # checkov:skip=CKV_AWS_23: "Ensure every security group and rule has a description"
  name        = "${var.name_prefix}-deduct-lambda-sg-${local.workspace_safe}"
  description = "Security group for Deduct Lambda function"
  ...
  tags = {
    Name = "l${var.name_prefix}-deduct-lambda-sg-${local.workspace_safe}"
  }
}

resource "aws_security_group_rule" "egress_deduct_lambda_to_sm_endpoint" {
  # checkov:skip=CKV_AWS_23: "Ensure every security group and rule has a description"
  type              = "egress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
  security_group_id = aws_security_group.deduct_lambda_sg.id
